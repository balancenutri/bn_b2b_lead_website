...
import { Tab, type NavigationProps } from '../utils/libs';
          ...
          return (
            <button
              ...
              className={`relative px-6 py-3 rounded-full text-sm font-sans font-medium transition-all duration-300 ease-out ${
                isActive 
                  ? (isOfferings ? 'text-teal-deep' : 'text-white') 
            ...
            >
              {isActive && (
                <motion.div
                  layoutId="activeTab"
                  className={`absolute inset-0 rounded-full ${isOfferings ? 'bg-warmYellow' : 'bg-teal-base'}`}
                  initial={false}
                  transition={{ type: "spring", stiffness: 300, damping: 30 }}
                />
              )}
              <span className={`relative z-10 flex items-center gap-2 ${isOfferings && isActive ? 'font-bold' : ''}`}>
                {tab.label}
              </span>
            </button>
          );
